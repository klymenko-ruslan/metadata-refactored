...
import ti.metadata.domain.part.CartridgeController;
...
privileged aspect CartridgeController_Roo_Controller {
    ...
    public String CartridgeController.create(@Valid Cartridge cartridge, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        ...
        uiModel.asMap().clear();
        cartridge.persist();
        return "redirect:/cartridges/" + encodeUrlPathSegment(cartridge.getPartId().toString(), httpServletRequest);
    }
    ...
    public String CartridgeController.show(@PathVariable("partId") Long partId, Model uiModel) {
        uiModel.addAttribute("cartridge", Cartridge.findCartridge(partId));
    ...
    public String CartridgeController.updateForm(@PathVariable("partId") Long partId, Model uiModel) {
        populateEditForm(uiModel, Cartridge.findCartridge(partId));
        return "cartridges/update";
    }
    
    @RequestMapping(value = "/{partId}", method = RequestMethod.DELETE, produces = "text/html")
    public String CartridgeController.delete(@PathVariable("partId") Long partId, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        Cartridge cartridge = Cartridge.findCartridge(partId);
        cartridge.remove();
        uiModel.asMap().clear();
