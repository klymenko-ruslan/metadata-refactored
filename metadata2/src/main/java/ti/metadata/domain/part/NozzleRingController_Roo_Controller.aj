...
import ti.metadata.domain.part.NozzleRingController;
...
privileged aspect NozzleRingController_Roo_Controller {
    ...
    public String NozzleRingController.create(@Valid NozzleRing nozzleRing, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        ...
        uiModel.asMap().clear();
        nozzleRing.persist();
        return "redirect:/nozzlerings/" + encodeUrlPathSegment(nozzleRing.getPartId().toString(), httpServletRequest);
    }
    ...
    public String NozzleRingController.show(@PathVariable("partId") Long partId, Model uiModel) {
        uiModel.addAttribute("nozzlering", NozzleRing.findNozzleRing(partId));
    ...
    public String NozzleRingController.updateForm(@PathVariable("partId") Long partId, Model uiModel) {
        populateEditForm(uiModel, NozzleRing.findNozzleRing(partId));
        return "nozzlerings/update";
    }
    
    @RequestMapping(value = "/{partId}", method = RequestMethod.DELETE, produces = "text/html")
    public String NozzleRingController.delete(@PathVariable("partId") Long partId, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        NozzleRing nozzleRing = NozzleRing.findNozzleRing(partId);
        nozzleRing.remove();
        uiModel.asMap().clear();
