...
import com.turbointernational.metadata.entity.chlogsrc.ChangelogSourceId_;
import com.turbointernational.metadata.entity.chlogsrc.ChangelogSourceLink_;
import com.turbointernational.metadata.entity.chlogsrc.ChangelogSource_;
import com.turbointernational.metadata.entity.chlogsrc.Source_;
...
public class ChangelogSourceDao extends AbstractDao<ChangelogSource> {
    ...
    public Page<ChangelogSource> filter(Long sourceId, Long changelogId, String sortProperty, String sortOrder,
        ...
        if (sourceId != null) {
            lstPredicates.add(
                    cb.equal(root.get(ChangelogSource_.pk).get(ChangelogSourceId_.source).get(Source_.id), sourceId));
        }
        if (changelogId != null) {
            lstPredicates.add(cb.equal(root.get(ChangelogSource_.pk).get(ChangelogSourceId_.link)
                    .get(ChangelogSourceLink_.changelog).get(Changelog_.id), changelogId));
        }
