...
public class UserDao extends AbstractDao<User> {
    ...
    public Page<User> filterUsers(String displayName, String userName, String email, Long authProviderId,
        ...
        if (displayName != null) {
            lstPredicates.add(cb.like(cb.lower(root.get(et.getDeclaredSingularAttribute("name", String.class))),
                    "%" + displayName.toLowerCase() + "%"));
            numPredicates++;
        }
        if (userName != null) {
            lstPredicates.add(cb.like(cb.lower(root.get(et.getDeclaredSingularAttribute("username", String.class))),
                    "%" + userName.toLowerCase() + "%"));
            numPredicates++;
        }
        if (email != null) {
            lstPredicates.add(cb.like(cb.lower(root.get(et.getDeclaredSingularAttribute("email", String.class))),
                    "%" + email.toLowerCase() + "%"));
            numPredicates++;
        }
        if (authProviderId == null) {
            lstPredicates.add(cb.isNull(root.get("authProvider").get("id")));
            numPredicates++;
