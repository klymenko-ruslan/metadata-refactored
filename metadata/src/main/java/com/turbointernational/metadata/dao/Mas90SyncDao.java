...
import com.turbointernational.metadata.entity.Mas90Sync_;
...
public class Mas90SyncDao extends AbstractDao<Mas90Sync> {
    ...
    public Page<Mas90Sync> findHistory(int startPosition, int maxResults) {
        ...
        recsCriteriaQuery.where(criteriaBuilder.notEqual(recsRoot.get(Mas90Sync_.status), Mas90Sync.Status.IN_PROGRESS));
        recsCriteriaQuery.orderBy(criteriaBuilder.desc(recsRoot.get(Mas90Sync_.id)));
        ...
        recsCriteriaQuery.where(criteriaBuilder.notEqual(countRoot.get(Mas90Sync_.status), Mas90Sync.Status.IN_PROGRESS));
